// frontend/src/pages/Reportes/Costos.tsx
...
export default function ReporteCostos() {
  ...
  const cargarDatos = async (periodo?: string) => {
    try {
      setLoading(true);
      const data = await getCostos(periodo);
      setDatos(data);
    } catch (e: any) {
      setUiAlert({
        msg: e?.message || "Error cargando reporte de costos",
        type: "error",
      });
    } finally {
      setLoading(false);
    ...
    <Card sx={{ width: "100%", mb: 2 }}>
      <CardContent>
        <Typography variant="h6" sx={{ mb: 2 }}>
          Reporte de Costos: Periodos, Tipos y Proporciones
        </Typography>
        ...
        <Typography sx={{ mt: 2 }}>Resumen KPIs de costos:</Typography>
        <Box sx={{ display: "flex", gap: 5, mt: 2, flexWrap: "wrap" }}>
          <Box>
            ...
            <Typography variant="h6">
              {filtrados.reduce((a, b) => a + b.costos_fijos, 0)}
            </Typography>
          </Box>
          <Box>
            <Typography variant="body2" color="text.secondary">
            ...
            <Typography variant="h6">
              {filtrados.reduce((a, b) => a + b.costos_variables, 0)}
            </Typography>
          </Box>
          <Box>
            <Typography variant="body2" color="text.secondary">
            ...
            <Typography variant="h6">
              {filtrados.reduce((a, b) => a + b.extraordinarios, 0)}
            </Typography>
          </Box>
          <Box>
            <Typography variant="body2" color="text.secondary">
            ...
            <Typography variant="h6">
              {filtrados.reduce((a, b) => a + b.total, 0)}
            </Typography>
          </Box>
        </Box>
      </CardContent>
