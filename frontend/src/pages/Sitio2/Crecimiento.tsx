// src/pages/Sitio2/Crecimiento.tsx
...
import {
  getCrecimientoSitio2,
  addCrecimientoSitio2,
  updateCrecimientoSitio2,
  RegistroCrecimiento,
  NuevoRegistroCrecimiento,
} from "../../services/sitio2/Crecimiento";

const emptyForm: NuevoRegistroCrecimiento = {
  fecha: "",
  lote: "",
  corral: "",
  cantidad_pesada: 0,
...
export default function Sitio2Crecimiento() {
  const [registros, setRegistros] = useState<RegistroCrecimiento[]>([]);
  const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState<NuevoRegistroCrecimiento>(emptyForm);
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getCrecimientoSitio2();
        setRegistros(data);
      } catch (e: any) {
        console.error(e);
        setUiAlert({
          msg: e.message || "Error cargando pesajes",
          type: "error",
        });
      }
    })();
  ...
  const handleGuardar = async () => {
    try {
      ...
      if (editId === null) {
        await addCrecimientoSitio2(form);
        setUiAlert({
          msg: "Pesaje registrado correctamente",
          type: "success",
        });
      } else {
        await updateCrecimientoSitio2(editId, form);
        setUiAlert({
    ...
    <Card sx={{ mb: 3, width: "100%", maxWidth: 980, boxSizing: "border-box" }}>
      <CardContent>
        <Typography variant="h6" sx={{ mb: 2 }}>
          Seguimiento de Crecimiento y Pesajes – Sitio 2
        </Typography>
        <Button
          variant="contained"
