// src/pages/Sitio2/Mortalidad.tsx
...
import {
  getMortalidadSitio2,
  addMortalidadSitio2,
  updateMortalidadSitio2,
  RegistroMortalidad,
  NuevoRegistroMortalidad,
} from "../../services/sitio2/Mortalidad";

const emptyForm: NuevoRegistroMortalidad = {
  fecha: "",
  lote: "",
  corral: "",
  cantidad: 0,
...
export default function Sitio2Mortalidad() {
  const [registros, setRegistros] = useState<RegistroMortalidad[]>([]);
  const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState<NuevoRegistroMortalidad>(emptyForm);
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getMortalidadSitio2();
        setRegistros(data);
      } catch (e: any) {
        console.error(e);
        setUiAlert({
          msg: e.message || "Error cargando registros de mortalidad",
          type: "error",
        });
      }
    })();
  ...
  const handleGuardar = async () => {
    try {
      ...
      if (editId === null) {
        await addMortalidadSitio2(form);
        setUiAlert({
          msg: "Registro de baja agregado correctamente",
          type: "success",
        });
      } else {
        await updateMortalidadSitio2(editId, form);
        setUiAlert({
    ...
    <Card sx={{ mb: 3, width: "100%", maxWidth: 980, boxSizing: "border-box" }}>
      <CardContent>
        <Typography variant="h6" sx={{ mb: 2 }}>
          Mortalidad y Descartes – Sitio 2
        </Typography>
        <Button
          variant="contained"
            ...
            >
              ...
              <TextField
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    tipo: e.target.value as NuevoRegistroMortalidad["tipo"],
                  }))
                }
              >
