// src/pages/Sitio2/Comercializacion.tsx
...
import {
  ...
  RegistroVenta,
  NuevaVenta,
} from "../../services/sitio2/Comercializacion";

const emptyForm: NuevaVenta = {
  fecha: "",
  lote: "",
  corral: "",
  cantidad_vendida: 0,
...
export default function Sitio2Comercializacion() {
  const [ventas, setVentas] = useState<RegistroVenta[]>([]);
  const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState<NuevaVenta>(emptyForm);
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getVentasSitio2();
        setVentas(data);
      } catch (e: any) {
        console.error(e);
        setUiAlert({
          msg: e.message || "Error cargando ventas",
          type: "error",
        });
      }
    })();
  ...
  const handleGuardar = async () => {
    try {
      ...
      if (editId === null) {
        await addVentaSitio2(form);
        setUiAlert({
          msg: "Venta/Salida registrada correctamente",
          type: "success",
        });
      } else {
        await updateVentaSitio2(editId, form);
        setUiAlert({
    ...
    <Card sx={{ mb: 3, width: "100%", maxWidth: 980, boxSizing: "border-box" }}>
      <CardContent>
        <Typography variant="h6" sx={{ mb: 2 }}>
          Comercialización y Salidas – Sitio 2
        </Typography>
        <Button
          variant="contained"
