...
import {
  ...
  updateIngresoSitio3,
  IngresoEngorde3,
  NuevoIngresoEngorde3,
} from "../../services/sitio3/Ingreso";

const proveedorOptions = ["Recría interna", "Compra", "Otro"] as const;

const emptyForm: NuevoIngresoEngorde3 = {
  fecha: "",
  lote: "",
  cantidad: 0,
  peso_promedio: 0,
...
export default function Sitio3Ingreso() {
  const [ingresos, setIngresos] = useState<IngresoEngorde3[]>([]);
  const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState<NuevoIngresoEngorde3>(emptyForm);
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getIngresosSitio3();
        setIngresos(data);
      } catch (e: any) {
        setUiAlert({
          msg: e.message || "Error cargando ingresos Sitio 3",
          type: "error",
        });
      }
    })();
  ...
  const handleGuardar = async () => {
    try {
      ...
      if (editId === null) {
        await addIngresoSitio3(form);
        setUiAlert({
          msg: "Ingreso registrado correctamente",
          type: "success",
        });
      } else {
        await updateIngresoSitio3(editId, form);
        setUiAlert({
  ...
  const handleEdit = (id: number) => {
    const reg = ingresos.find((r) => r.id === id);
    if (!reg) return;
    ...
    <Card sx={{ mb: 3, width: "100%", maxWidth: 980, boxSizing: "border-box" }}>
      <CardContent>
        <Typography variant="h6" sx={{ mb: 2 }}>
          Ingreso de Animales a Engorde (Sitio 3)
        </Typography>
        <Button
          variant="contained"
