// src/pages/Sitio3/Nutricion.tsx
...
import {
  getNutricionSitio3,
  addNutricionSitio3,
  updateNutricionSitio3,
  RegistroNutricion3,
  NuevoRegistroNutricion3,
} from "../../services/sitio3/Nutricion";

const emptyForm: NuevoRegistroNutricion3 = {
  fecha: "",
  corral: "",
  lote: "",
  dieta: "",
...
export default function Sitio3Nutricion() {
  const [registros, setRegistros] = useState<RegistroNutricion3[]>([]);
  const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState<NuevoRegistroNutricion3>(emptyForm);
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getNutricionSitio3();
        setRegistros(data);
      } catch (e: any) {
        setUiAlert({
          msg: e.message || "Error cargando registros nutricionales Sitio 3",
          type: "error",
        });
      }
    })();
  ...
  const handleGuardar = async () => {
    try {
      ...
      if (editId === null) {
        await addNutricionSitio3(form);
        setUiAlert({
          msg: "Registro nutricional agregado correctamente",
          type: "success",
        });
      } else {
        await updateNutricionSitio3(editId, form);
        setUiAlert({
    ...
    <Card sx={{ mb: 3, width: "100%", maxWidth: 980, boxSizing: "border-box" }}>
      <CardContent>
        <Typography variant="h6" sx={{ mb: 2 }}>
          Nutrición y Alimentación – Sitio 3
        </Typography>
        <Button
          variant="contained"
