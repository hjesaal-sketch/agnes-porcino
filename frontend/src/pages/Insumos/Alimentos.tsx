// src/pages/Insumos/Alimentos.tsx
import React, { useState, useEffect } from "react";
import {
  Card,
  CardContent,
  Typography,
  Box,
  Button,
  Dialog,
  DialogTitle,
  DialogContent,
  DialogActions,
  TextField,
  Snackbar,
  Alert as MuiAlert,
} from "@mui/material";
import {
  getAlimentos,
  addAlimento,
  updateAlimento,
  deleteAlimento,
  Alimento,
} from "../../services/insumos/Alimentos";

const emptyForm: Omit<Alimento, "id"> = {
  tipo: "",
  fase: "",
  proteina: "",
  energiaKcal: "",
  presentacion: "",
  proveedor: "",
  lote: "",
  cantidad: 0,
  unidad: "kg",
  stock: 0,
  vencimiento: "",
  observaciones: "",
};

type UiAlertState = { msg: string; type: "success" | "error" } | null;

export default function InsumosAlimentos() {
  const [alimentos, setAlimentos] = useState<Alimento[]>([]);
  const [form, setForm] = useState<Omit<Alimento, "id">>(emptyForm);
  const [showDialog, setShowDialog] = useState(false);
  const [editId, setEditId] = useState<number | null>(null);
  const [uiAlert, setUiAlert] = useState<UiAlertState>(null);

  useEffect(() => {
    (async () => {
      const data = await getAlimentos();
      setAlimentos(data);
    })();
  }, []);

  const limpiarForm = () => setForm(emptyForm);

  const handleGuardar = async () => {
    try {
      if (!form.tipo || !form.lote || form.cantidad < 1) {
        setUiAlert({
          msg: "Completa tipo, cantidad y lote",
          type: "error",
        });
        return;
      }
      if (editId !== null) {
        await updateAlimento(editId, form);
        setUiAlert({
          msg: "Alimento actualizado correctamente",
          type: "success",
        });
      } else {
        await addAlimento(form);
        setUiAlert({
          msg: "Alimento agregado correctamente",
          type: "success",
        });
      }
      const data = await getAlimentos();
      setAlimentos(data);
      setShowDialog(false);
      setEditId(null);
      limpiarForm();
    } catch (err: any) {
      setUiAlert({ msg: err.message || "Error", type: "error" });
    }
  };

  const handleEditar = (id: number) => {
    const alimento = alimentos.find((a) => a.id === id);
    if (alimento) {
      const { id: _id, ...rest } = alimento;
      setForm(rest);
      setEditId(id);
      setShowDialog(true);
    }
  };

  const handleEliminar = async (id: number) => {
    await deleteAlimento(id);
    const data = await getAlimentos();
    setAlimentos(data);
    setUiAlert({ msg: "Registro eliminado", type: "success" });
  };

  const handleCloseSnackbar = () => setUiAlert(null);

  return (
    <Card sx={{ mb: 3, maxWidth: 1050, margin: "0 auto" }}>
      <CardContent>
        <Typography variant="h6" sx={{ mb: 2 }}>
          Inventario y Control de Alimentos
        </Typography>
        <Button
          variant="contained"
          sx={{ mb: 2, bgcolor: "#169b62" }}
          onClick={() => {
            setShowDialog(true);
            limpiarForm();
            setEditId(null);
          }}
        >
          Registrar Alimento
        </Button>

        {/* Contenedor con scroll horizontal como en Medicamentos */}
        <Box sx={{ width: "100%", overflowX: "auto" }}>
          <table
            style={{
              minWidth: 1200, // ancho mínimo para que no se congestione el header
              borderCollapse: "collapse",
              background: "#fff",
              marginBottom: 24,
              boxShadow: "0 1px 8px #0001",
              borderRadius: 10,
              overflow: "hidden",
            }}
          >
            <thead>
              <tr style={{ background: "#169b62", color: "#fff", height: 33 }}>
                <th>Tipo</th>
                <th>Fase</th>
                <th>Proteína (%)</th>
                <th>Energía (Kcal)</th>
                <th>Presentación</th>
                <th>Proveedor</th>
                <th>Lote</th>
                <th>Cantidad</th>
                <th>Unidad</th>
                <th>Stock</th>
                <th>Vencimiento</th>
                <th>Observaciones</th>
                <th>Acciones</th>
              </tr>
            </thead>
            <tbody>
              {alimentos.map((a) => (
                <tr key={a.id} style={{ borderBottom: "1px solid #eee" }}>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.tipo}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.fase}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.proteina}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.energiaKcal}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.presentacion}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.proveedor}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.lote}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.cantidad}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.unidad}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.stock}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.vencimiento}
                  </td>
                  <td style={{ textAlign: "center", fontSize: 13 }}>
                    {a.observaciones}
                  </td>
                  <td style={{ textAlign: "center" }}>
                    <Button
                      size="small"
                      sx={{
                        background: "#169b62",
                        color: "#fff",
                        px: 2,
                        fontSize: 13,
                        borderRadius: 1,
                        fontWeight: 700,
                        mr: 1,
                      }}
                      onClick={() => handleEditar(a.id)}
                    >
                      Editar
                    </Button>
                    <Button
                      size="small"
                      sx={{
                        background: "#b52424",
                        color: "#fff",
                        px: 2,
                        fontSize: 13,
                        borderRadius: 1,
                        fontWeight: 700,
                      }}
                      onClick={() => handleEliminar(a.id)}
                    >
                      Eliminar
                    </Button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </Box>

        <Dialog
          open={showDialog}
          onClose={() => {
            setShowDialog(false);
            setEditId(null);
            limpiarForm();
          }}
          maxWidth="md"
          fullWidth
        >
          <DialogTitle>
            {editId ? "Editar Alimento" : "Registrar Alimento"}
          </DialogTitle>
          <DialogContent sx={{ display: "flex", flexWrap: "wrap", gap: 2 }}>
            <TextField
              label="Tipo"
              value={form.tipo}
              onChange={(e) =>
                setForm((f) => ({ ...f, tipo: e.target.value }))
              }
            />
            <TextField
              label="Fase"
              value={form.fase}
              onChange={(e) =>
                setForm((f) => ({ ...f, fase: e.target.value }))
              }
            />
            <TextField
              label="Proteína (%)"
              value={form.proteina}
              onChange={(e) =>
                setForm((f) => ({ ...f, proteina: e.target.value }))
              }
            />
            <TextField
              label="Energía (Kcal)"
              value={form.energiaKcal}
              onChange={(e) =>
                setForm((f) => ({ ...f, energiaKcal: e.target.value }))
              }
            />
            <TextField
              label="Presentación"
              value={form.presentacion}
              onChange={(e) =>
                setForm((f) => ({ ...f, presentacion: e.target.value }))
              }
            />
            <TextField
              label="Proveedor"
              value={form.proveedor}
              onChange={(e) =>
                setForm((f) => ({ ...f, proveedor: e.target.value }))
              }
            />
            <TextField
              label="Lote"
              value={form.lote}
              onChange={(e) =>
                setForm((f) => ({ ...f, lote: e.target.value }))
              }
            />
            <TextField
              label="Cantidad"
              type="number"
              value={form.cantidad}
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  cantidad: Number(e.target.value) || 0,
                }))
              }
            />
            <TextField
              label="Unidad"
              value={form.unidad}
              onChange={(e) =>
                setForm((f) => ({ ...f, unidad: e.target.value }))
              }
            />
            <TextField
              label="Stock"
              type="number"
              value={form.stock}
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  stock: Number(e.target.value) || 0,
                }))
              }
            />
            <TextField
              label="Vencimiento"
              type="date"
              InputLabelProps={{ shrink: true }}
              value={form.vencimiento}
              onChange={(e) =>
                setForm((f) => ({ ...f, vencimiento: e.target.value }))
              }
            />
            <TextField
              label="Observaciones"
              value={form.observaciones}
              onChange={(e) =>
                setForm((f) => ({ ...f, observaciones: e.target.value }))
              }
              fullWidth
            />
          </DialogContent>
          <DialogActions>
            <Button
              onClick={handleGuardar}
              variant="contained"
              sx={{ bgcolor: "#169b62" }}
            >
              Guardar
            </Button>
            <Button
              onClick={() => {
                setShowDialog(false);
                setEditId(null);
                limpiarForm();
              }}
              variant="outlined"
            >
              Cancelar
            </Button>
          </DialogActions>
        </Dialog>

        <Snackbar
          open={!!uiAlert}
          autoHideDuration={3200}
          onClose={handleCloseSnackbar}
        >
          {uiAlert ? (
            <MuiAlert
              onClose={handleCloseSnackbar}
              severity={uiAlert.type}
              sx={{ width: "100%" }}
            >
              {uiAlert.msg}
            </MuiAlert>
          ) : undefined}
        </Snackbar>
      </CardContent>
    </Card>
  );
}
