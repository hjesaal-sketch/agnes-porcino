// src/services/maternidadAlertasService.ts
...
const STORAGE_KEY = "maternidad_alertas";

function getAll(): AlertaMaternidad[] {
  const raw = localStorage.getItem(STORAGE_KEY);
  return raw ? JSON.parse(raw) : [];
}

function saveAll(alertas: AlertaMaternidad[]) {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(alertas));
}

export function addAlerta(alerta: Omit<AlertaMaternidad, "id">) {
  const nueva: AlertaMaternidad = { ...alerta, id: crypto.randomUUID() };
...
export function updateAlerta(id: string, alerta: Omit<AlertaMaternidad, "id">) {
  const alertas = getAll();
